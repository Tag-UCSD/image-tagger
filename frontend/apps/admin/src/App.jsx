...
import { admin, demoAccess, Header, Button } from '@shared';
...
export default function AdminApp() {
    ...
    const loadData = useCallback(async () => {
        ...
        setErrorMsg(null);
        try {
            const budgetData = await admin.getBudget();
            setBudget(budgetData);
            setKillActive(!!budgetData?.is_kill_switched);
            setAppState('loaded');
        } catch (err) {
            if (err.status === 403) {
    ...
    const handleKillToggle = (resp) => {
        // Kill-switch response only carries is_kill_switched — budget fields come from getBudget().
        setBudget(prev => prev ? { ...prev, is_kill_switched: !!resp?.is_kill_switched } : prev);
        setKillActive(!!resp?.is_kill_switched);
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100 pb-10">
            ...
            <div className="px-4 py-6 sm:px-8 max-w-5xl mx-auto space-y-6">
                ...
                {appState === 'error' && <ErrorView message={errorMsg} onRetry={loadData} />}
                ...
                {appState === 'loaded' && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            <KillSwitch active={killActive} onToggle={handleKillToggle} />
                        </div>
                    </div>
                )}
            </div>
...
function LoadingView() {
    return (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-500">
            ...
            <p className="text-sm">Loading cockpit…</p>
        </div>
    );
}
...
function ErrorView({ message, onRetry }) {
    return (
        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
            ...
            <h3 className="text-base font-semibold text-gray-900">Could not load admin data</h3>
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 text-center">
                ...
                <p className="text-sm text-gray-600 max-w-sm">
                    This cockpit is only available to users with the admin role.
                </p>
            </div>
        </div>
    );
