...
python3 scripts/hollow_repo_guard.py || { echo "❌ Hollow Repo Guard failed"; exit 1; }

echo "🧪 Running program integrity guard"
python3 scripts/program_integrity_guard.py || { echo "❌ Program Integrity Guard failed"; exit 1; }
echo "🧯 Running syntax guard"

python3 scripts/syntax_guard.py || { echo "❌ Syntax Guard failed"; exit 1; }

echo "🪝 Running critical import guard"

python3 scripts/critical_import_guard.py || { echo "❌ Critical Import Guard failed"; exit 1; }
...
docker-compose -f deploy/docker-compose.yml exec -T api \
  python3 scripts/smoke_science.py || echo "[install] smoke_science (second pass) failed – continuing as advisory."
